...
class StoreVehiculeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'immatriculation' => ['required', 'string', 'regex:/^[A-Za-z]{2}[-\s]?[0-9]{3}[-\s]?[A-Za-z]{2}$/','unique:vehicule,immatriculation'],
    ...
    protected function prepareForValidation(): void
    {
        if ($this->has('immatriculation')) {
            $this->merge([
                'immatriculation' => strtoupper(preg_replace('/[-\s]/', '', $this->immatriculation)),
            ]);
        }
    }
