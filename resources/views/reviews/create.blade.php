        ...
        <div class="max-w-3xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                ...
                <form action="{{ route('reviews.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 space-y-8">
                    ...
                    <div>
                        <h3 class="font-bold text-noir mb-4">Critères détaillés</h3>
                        <div class="space-y-4">
                            @foreach(['Ponctualité', 'Courtoisie', 'Qualité de conduite', 'Propreté du véhicule'] as $criteria)
                            <div class="flex items-center justify-between">
                                <span class="text-gris1">{{ $criteria }}</span>
                                <div x-data="{ rating: 0, hover: 0 }" class="flex gap-1">
                                    <input type="hidden" name="criteria[{{ $criteria }}]" :value="rating">
                                    <template x-for="star in 5">
                                        <button type="button" @click="rating = star" @mouseenter="hover = star" @mouseleave="hover = 0" class="focus:outline-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" :class="(hover || rating) >= star ? 'text-yellow-400 fill-current' : 'text-gray-200'" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                            </svg>
                                        </button>
                                    </template>
